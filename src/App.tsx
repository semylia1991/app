...
import { Camera, AlertCircle, ShieldCheck, Leaf, Info, Sparkles, AlertTriangle, Zap, Clock, Rimport ReactMarkdown from 'react-markdown';
...
import { LegalModal, PrivacyPolicyContent, ImpressumContent } from './components/LegalModals'import { fetchProductImage } from './lib/productImage';
...
function UsageSection({ text }: { text: string }) {
 ...
 const emoji = rawLabel.match(/^(\p{Emoji_Presentation}|\p{Emoji}\uFE0F)/u)?.[0] ??  const label = rawLabel.replace(/^(\p{Emoji_Presentation}|\p{Emoji}\uFE0F)\s*/u, '') const body = block.slice(colonIdx + 1).trim();
 return (
 <p key={i}>
...
function BenefitsSection({ text }: { text: string }) {
 ...
 const isHeader = /[：:]$/.test(header) || /^[\p{Emoji_Presentation}\p{Emoji}\uFE0F]/u if (isHeader) {
 return (
 <div key={i}>
...
function ProductHeroImage({ name, brand, userPhoto }: { name: string; brand: string; userPhot const [src, setSrc] = useState<string | null>(null);
 ...
 <div className="w-28 h-28 rounded-sm border border-[#D4C3A3] bg-[#F5F0E8] overflow-hidd {state === 'loading' && (
 <div className="w-6 h-6 rounded-full border-2 border-[#B89F7A]/30 border-t-[#B89F7A )}
 {state === 'loaded' && src && (
 <img
 src={src}
 alt={name}
...
export default function App() {
 ...
 const photoToSave = previewUrl; // capture before clearing
 ...
 <div className="fixed top-0 left-0 w-full h-32 bg-gradient-to-b from-[#B89F7A]/10 to-tr <div className="fixed bottom-0 left-0 w-full h-32 bg-gradient-to-t from-[#B89F7A]/10 to <header className="pt-6 pb-4 px-4 text-center relative">
 {/* Auth row */}
 <div className="flex items-center justify-between gap-2 mb-3">
 <img src={logo} alt="logo" style={{ width: 40, height: 40, objectFit: 'contain' }}  <div className="flex items-center gap-2">
 {user && (
 <ScanHistory
 user={user}
 lang={lang}
 ...
 <div
 className="relative aspect-[3/2] border-2 border-dashed border-[#D4C3A3] roun onClick={() => fileInputRef.current?.click()}
 >
 {previewUrl ? (
 <img src={previewUrl} alt="Preview" className="absolute inset-0 w-full h-fu ) : (
 <div className="text-center p-6 flex flex-col items-center text-[#B89F7A]">
 <Camera size={48} strokeWidth={1} className="mb-4" />
 <span className="font-serif text-sm tracking-widest uppercase">{t[lang].u </div>
 )}
 <input
 type="file"
 ref={fileInputRef}
 ...
 className="peer appearance-none w-4 h-4 border border-[#B89F7A] round />
 <svg className="absolute w-3 h-3 text-white pointer-events-none opacity </div>
 <span className="text-xs text-[#4A4A4A] leading-relaxed group-hover:text- {t[lang].consent}
 </span>
 </label>
 {error && (
 <div className="text-red-800 text-xs bg-red-50 p-3 border border-red-200  <AlertCircle size={14} className="shrink-0 mt-0.5" />
 <span>{error}</span>
 </div>
 )}
 <button
 ...
 className="w-full py-4 regency-button disabled:opacity-50 disabled:cursor >
 {isAnalyzing ? (
 <>
 ...
 <h2 className="text-sm font-serif tracking-[0.2em] text-[#B89F7A] uppercase m {t[lang].ingredientAnalysis}
 </h2>
 <ProductHeroImage name={result.productName} brand={result.brand} userPhoto={s <h3 className="text-2xl font-serif text-[#2C3E50] mb-1">{result.productName}< <p className="text-sm text-[#4A4A4A] italic">{result.brand}</p>
 {isTranslating && (
 <div className="mt-2 flex items-center justify-center gap-2 text-[#B89F7A]  <Loader2 size={12} className="animate-spin" />
 {t[lang].translating}
 </div>
 )}
 </div>
 <div className="space-y-2">
 <CollapsibleSection title={t[lang].analysis} icon={<ShieldCheck size={20} />} <div className="prose prose-sm prose-stone max-w-none">
 ...
 <li key={idx} className="flex items-start gap-2 py-1 border-b border-[# <span className="text-base shrink-0 mt-0.5">{ing.status}</span>
 <div className="flex flex-col">
 <span className="font-semibold text-[#2C3E50] text-xs uppercase tra <span className="text-xs text-[#4A4A4A]">{ing.description}</span>
 </div>
 </li>
 ))}
 </ul>
 ...
 <CollapsibleSection title={t[lang].sideEffects} icon={<AlertTriangle size={20 <BenefitsSection text={result.sideEffects} />
 </CollapsibleSection>
 <CollapsibleSection title={t[lang].warnings} icon={<AlertCircle size={20} />} <div className="prose prose-sm prose-stone max-w-none">
 ...
 <CollapsibleSection title={t[lang].alternatives} icon={<RefreshCw size={20} / <AlternativesSection alternatives={result.alternatives} />
 </CollapsibleSection>
 </div>
 <AskAI lang={lang} context={result} />
 <div className="mt-8 pt-6 border-t border-[#D4C3A3] space-y-4">
 <div className="bg-[#B89F7A]/5 p-4 rounded-sm border border-[#B89F7A]/20 text <p><strong>Transparency:</strong> {t[lang].aiTransparency}</p>
 ...
 <button onClick={() => setIsPrivacyOpen(true)} className="hover:text-[#2C3E50] tran {t[lang].privacyPolicy}
 </button>
 <span>|</span>
 <button onClick={() => setIsImpressumOpen(true)} className="hover:text-[#2C3E50] tr {t[lang].impressum}
 </button>
 </div>
 </footer>
